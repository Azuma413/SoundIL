...
class SoundTask:
    ...
    def _build_scene(self):
        ...
        # 箱を追加
        self.box = self.scene.add_entity(gs.morphs.URDF(file="URDF/box/box.urdf"))
        # フロントカメラを追加
    ...
    def compute_reward(self):
        # Get z position of cubeA
        pos = self.cubeA.get_pos().cpu().numpy()
        z_height = pos[-1]
        reward = float(z_height > 0.1)
        return reward
    ...
    def get_obs(self):
        ...
        gripper = self.franka.get_dofs_position()[0, 7:9].cpu().numpy()
...
class SoundCamera:
    ...
    def render(self):
        ...
        return np.random.randint(0, 255, (self.observation_height, self.observation_width, 3), dtype=np.uint8),
